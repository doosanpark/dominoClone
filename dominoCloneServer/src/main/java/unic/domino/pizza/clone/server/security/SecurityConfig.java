package unic.domino.pizza.clone.server.security;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        // form Login 설정을 진행
        http.formLogin()
                .loginPage("/login")	// 커스텀 로그인 페이지 경로와 로그인 인증 경로를 등록
                .defaultSuccessUrl("/") // 로그인 인증을 성공하면 이동하는 페이지를 등록
                .permitAll();

        // 로그아웃 설정을 진행
        http.logout()
                .logoutRequestMatcher(new AntPathRequestMatcher("/logout")) //logoutRequestMatcher(new AntPathRequestMatcher("path"))
    ...
    // AuthenticationManagerBuilder : AuthenticationManager를 생성. AuthenticationManager는 사용자 인증을 담당
    @Override
    public void configure(AuthenticationManagerBuilder auth) throws Exception {
        auth.userDetailsService(memberService).passwordEncoder(passwordEncoder());
        //auth.userDetailsService(service)에 org.springframework.security.core.userdetails.UserDetailsService 인터페이스를 구현한 Service를 넘겨야함
    }
